...
def test_compare_csv_outputs_teams():
    ...
    comparison = expected[['Team','date','league']].merge(actual, on=['Team','date','league'], how='inner')
    ...
    comparison = comparison.sort_values(['Team','date','league']).reset_index(drop=True)
